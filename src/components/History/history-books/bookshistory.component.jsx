            ...
            {BookHistory && BookHistory.map((item) => {
                return(
                    <div key={item.$id} className="flex justify-evenly border border-black rounded-md w-3/4 m-auto mb-10">
                        <p>{item.added_book.title.slice(0,10) + (item.added_book.title.length > 10 ? '...' : '')}</p>
                        <p>{item.added_book.author.slice(0,10) + (item.added_book.author.length > 10 ? '...' : '')}</p>
                        <p>{item.added_book.stock}</p>
